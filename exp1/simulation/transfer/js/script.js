...
const xval = [0, 1, 2, 3, 4, 5, 5.5, 6];
const data = {
  vge100: [0, 0, 0, 0, 0, 0.2, 75],
};
const datafinal = {
  vge100: [0, 0, 0, 0, 0, 0.2, 5.9, 75],
};
    ...
    if (new_reading_vge) {
      $(
        '<div class="custom-menu"><div class="name-element"><div class="name-element"><div style="display: flex; justify-content: end; position: relative;top: -4px;height: 28px; margin-bottom:2px; ;"><button class="submit fa fa-times cross-btn" id="submit-' +
          ...
          dc_defult["d100"] +
          " " +
          dc_defult_enabled["d100"] +
          ' value="10">10 V</option></select>  ' +
          '</div><div style="display: flex; justify-content: end; padding-right: 13px"><button type="submit"  class="set-value-btn" style="border-radius: 20px">Set Value</button></div></form></div>'
      )
        .appendTo("body")
        .css({ top: event.pageY + "px", left: event.pageX + 10 + "px" });
    ...
    if (current_reading_no < 4) {
      ...
      if (first_reading_flag) {
        ...
        tab.innerHTML =
          ...
          <th  colspan="3">V<sub>CE</sub>&nbsp;=&nbsp;${a}&nbsp;V</th>
          
      </tr>
      <tr id="row-3-ic">
      <th>V<sub>GE</sub>&nbsp;(V)</th>
...
function check_vce_values() {
  ...
  if (major_count % 7 == 0 && major_count!=0) {
    openPopup(
      "new-img/404-warning.png",
      "You can add only seven readings in the table",
      "22px"
    );
  } else {
    if (check_flag) {
      previous_readings.push(parseFloat(a));
      new_reading = true;
      previous_reading_current = false;
      if (parseFloat(a) == 5) {
        dc_vcc_enabled["d55"] = " ";
      } else {
        dc_vcc_enabled["d" + (parseFloat(a) + 1) * 10] = " ";
      }
      if (parseFloat(a) == 5.5) {
        dis55();
        dc_vcc_enabled["d60"] = " ";
...
function display_readings() {
  if (new_reading_table) {
    ...
    tab.innerHTML =
      tab.innerHTML +
      `<tr id="row-${count}-reading-value"><td>${
        major_count + 1
      }</td><td>${count}</td><td>${
        data["vge" + values["DC1"]["value"] * 10][count]
      }</td></tr>`;
    if (count == 6) {
      new_reading_vge = true;
      ...
      plot_data_points.push({
        ...
        y: datafinal["vge100"],
        mode: "lines",
        name: `V<sub>GE</sub> = ${values["DC1"]["value"]}V`,
        marker: {
          color: colors.pop(),
        },
      });
...
function dis55() {
  ...
  tab.innerHTML =
    tab.innerHTML +
    `<tr id="row-88-reading-value"><td>${
      major_count + 1
    }</td><td>5.5</td><td>5.9</td></tr>`;
  major_count += 1;
}
